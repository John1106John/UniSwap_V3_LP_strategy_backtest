...
def filtered_data(data,start_date,end_date) :
    ...
    start_timestamp = int(datetime_obj.replace(tzinfo=datetime.timezone.utc).timestamp())
    datetime_obj = datetime.datetime.strptime(end_date, '%Y-%m-%d %H:%M:%S')
    end_timestamp = int(datetime_obj.replace(tzinfo=datetime.timezone.utc).timestamp())
